...
@pytest.fixture
def lexer():
    from p_unity.BASIC import BasicLexer
    return BasicLexer()
...
def interpreter(request):
    from p_unity.BASIC import Engine
...
def test_interpreter_assignment(interpreter):
    assert interpreter.eFORTH.root.memory['a'] == 2
...
def test_interpreter_simple_arithmetic(interpreter):
    assert interpreter.eFORTH.root.memory['a'] == 3
